...
def compute_pixelwise_retrieval_metrics(anomaly_segmentations, ground_truth_masks):
    anomaly_segmentations = np.stack(anomaly_segmentations) if isinstance(anomaly_segmentations, list) else anomaly_segmentations
    ground_truth_masks = np.stack(ground_truth_masks) if isinstance(ground_truth_masks, list) else ground_truth_masks
    ...
    mean_AP = metrics.average_precision_score(flat_ground_truth_masks.astype(int), flat_anomaly_segmentations)
...
def compute_pro(anomaly_map: ndarray, gt_mask: ndarray, label: ndarray, num_th: int = 200):
    ...
    for anomaly_mask, mask in zip(current_amap, current_mask):
        ...
        min_th, max_th = anomaly_mask.min(), anomaly_mask.max()
        delta = (max_th - min_th) / num_th

        for th in np.arange(min_th, max_th, delta):
            binary_amaps = (anomaly_mask > th).astype(np.bool_)
...
# The other functions (`save_image`, `save_video_segmentations`, `cv2_ima_save`) remain unchanged.
