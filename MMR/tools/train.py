...
def train(cfg=None):
    ...
    # get train dataloaders (include each category)
    train_dataloaders = get_dataloaders(cfg=cfg, mode='train')
    # get test dataloaders (include each category)
    ...
    for idx, individual_dataloader in enumerate(train_dataloaders):
        ...
        if cfg.TRAIN.method in ['MMR']:
            ...
            param_groups = add_weight_decay(mmr_base, cfg.TRAIN_SETUPS.weight_decay)
        ...
        # Use SAVE_DIR or default to "./checkpoints"
        save_dir = getattr(cfg, "SAVE_DIR", "./checkpoints")
        os.makedirs(save_dir, exist_ok=True)
        model_save_path = os.path.join(save_dir, f"{individual_dataloader.name}_MMR_model.pth")

        # Save the model
        torch.save({
            "cur_model_state_dict": cur_model.state_dict(),
            "mmr_base_state_dict": mmr_base.state_dict(),
